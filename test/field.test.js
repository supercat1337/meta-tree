...
test('constructor validates field name', t => {
    t.throws(() => new MetaField('user name'), { message: /Invalid field name/ });
    t.throws(() => new MetaField('user@name'), { message: /Invalid field name/ });
    t.notThrows(() => new MetaField('user_name'));
    t.notThrows(() => new MetaField('user.name'));
});
...
test('stringify optional field with default', t => {
    const field = new MetaField('role', true, 'guest');
    const str = field.stringify();
    t.is(str, '[role="guest"]');
});

test('stringify optional field without default', t => {
    const field = new MetaField('role', true, null);
    const str = field.stringify();
    t.is(str, '[role]');
});

test('stringify escapes backslashes and newlines in description', t => {
    const field = new MetaField('note', false, null, 'Line1\nLine2\\Backslash');
    const str = field.stringify();
    t.is(str, 'note    // Line1\\nLine2\\\\Backslash');
});

test('clone creates independent copy', t => {
    const original = new MetaField('test', true, 'default', 'desc');
